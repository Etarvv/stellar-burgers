...
describe('burgerConstructor slice', () => {
  ...
  test('should handle moveIngredient', () => {
    ...
    let state = constructorReducer(initialState, addIngredient(filling1));
    state = constructorReducer(state, addIngredient(filling2));
    expect(state.ingredients.map(i => i.name)).toEqual(['Котлета', 'Сыр']);

    const action = moveIngredient({ from: 0, to: 1 });
    const newState = constructorReducer(state, action);
    expect(newState.ingredients.map(i => i.name)).toEqual(['Сыр', 'Котлета']);

    const reverseAction = moveIngredient({ from: 1, to: 0 });
    const finalState = constructorReducer(newState, reverseAction);
    expect(finalState.ingredients.map(i => i.name)).toEqual(['Котлета', 'Сыр']);
  });
